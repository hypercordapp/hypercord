...
// Online themes have no separate "enabled" list here - being present in
// Settings.themeLinks is what makes one active, so cycling/toggling operates
// on that array directly, scoped to the online ids this plugin knows about.
function applyTheme(theme: ThemeItem) {
    const isLocal = theme.type === "local";
    Settings.enabledThemes = isLocal ? [theme.id] : [];

    if (!isLocal) {
        const onlineIds = new Set(themeList.filter(t => t.type === "online").map(t => t.id));
        Settings.themeLinks = [...Settings.themeLinks.filter((l: string) => !onlineIds.has(l)), theme.id];
    }
}
...
function findCurrentThemeIndex(): number {
    ...
    const idx = themeList.findIndex(
        t => (t.type === "local" && t.id === enabledLocal) || (t.type === "online" && Settings.themeLinks?.includes(t.id)),
    );
...
function toggleCurrentTheme(enable: boolean) {
    ...
    const isLocal = theme.type === "local";
    const arr = isLocal ? Settings.enabledThemes : Settings.themeLinks;
    ...
    if (isLocal) {
        ...
        Settings.themeLinks = enable ? [...arr, theme.id] : arr.filter((t: string) => t !== theme.id);
    }
}
